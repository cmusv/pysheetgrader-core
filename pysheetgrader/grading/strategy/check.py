...
class CheckStrategy(BaseStrategy):
    ...
    def get_submitted_value(self):
        ...
        return self.get_formula_value(self.sub_sheet_compute, key_raw_formula, self.grading_rubric.parse_excel)
    ...
    def get_key_value(self, key_coord):
        ...
        # else check the value of the given cell in the key
            ...
            return self.get_formula_value(self.key_sheet_raw, key_raw_formula, self.grading_rubric.parse_excel)    
    ...
    def check_correct(self, sub_cell_value, key_cell_value, key_coord):
        ...
        if(key_coord and key_coord in self.grading_rubric.alt_cells):
            # we must get the sub cell value again if it is a formula
             # TODO: consider?
            try:
                sub_cell_value = self.get_formula_value(self.sub_sheet_compute, self.key_sheet_raw[key_coord].value, self.grading_rubric.parse_excel)
            except:
